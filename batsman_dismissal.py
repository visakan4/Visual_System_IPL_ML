...
def getYear(season_id):
    return season_data.at[season_data[season_data['Season_Id'] == season_id].index[0],'Match_Date']

# isPlaying
# param - players_ball_by_ball, player_id
# return true if player got out, else false
...
# modeOfDismisallCount
# param - match_ball_by_ball,
# get mode of dismissal of the player, season_player_out
...
# getValues
# iterate season by group and determine the necessary values
# create a hash for each season and fill in the necessary values
# push the hash into a array
# return the array
...
def getValues():
    for season_id, details_in_season in details_by_season:
        ...
        season_player_out['season_year'] = getYear(season_id)
...
# getPlayers
# param - player_id_selected
# initialize the necessary variables
# read necessary files into DataFrames
# group the dataFrames by match_id, season_id and player_id
# getValues method is invoked
...
def work_flow(player_id_selected):
    ...
    player_all_matches = ps.read_csv(
        filepath_or_buffer='./indian-premier-league-csv-dataset/Player_Match.csv',
        sep=',')
